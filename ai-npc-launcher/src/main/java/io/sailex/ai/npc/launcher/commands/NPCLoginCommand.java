...
public class NPCLoginCommand {
    ...
    private void login(String email, String password, ServerPlayerEntity source) {
        try {
            ...
            StepFullJavaSession.FullJavaSession javaSession = MinecraftAuth.JAVA_CREDENTIALS_LOGIN.getFromInput(httpClient, new StepCredentialsMsaCode.MsaCredentials(email, password));
    ...
    private void saveAccount(StepFullJavaSession.FullJavaSession fullJavaSession, ServerPlayerEntity source) {
        ...
        try {
            validatedAccount = launcher.getAccountManager().getAccountValidator().validate(fullJavaSession);
        } catch (AuthException e) {
            source.sendMessage(LogUtil.formatError("Failed to validate account!"));
            return;
        }
        source.sendMessage(LogUtil.formatInfo("Logged into account " + validatedAccount.getName() + " successfully! Now you can spawn a NPC with this account."));
